...
RSpec.describe WorkPackage::SemanticIdentifier do
  ...
  describe "semantic_identifier_fields_consistent validation" do
    ...
    context "when semantic mode is inactive" do
      before { allow(Setting::WorkPackageIdentifier).to receive(:semantic_mode_active?).and_return(false) }

      it "skips the validation even when fields are inconsistent" do
        wp.identifier = "MYPROJ-1"
        expect(wp).to be_valid
      end
    end
