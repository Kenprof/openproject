...
RSpec.describe API::V3::WorkPackages::WorkPackageRepresenter do
  ...
  let(:budget) { build_stubbed(:budget, project: workspace) }
  let(:duration) { nil }
  ...
  let(:workspace) { build_stubbed(:project_with_types, phases: project_phases) }
  ...
  describe "_links" do
    ...
    describe "linked relations" do
      ...
      describe "ancestors" do
        let(:root) { build_stubbed(:work_package, project: workspace) }
        let(:intermediate) do
          build_stubbed(:work_package, parent: root, project: workspace)
        end
      ...
      describe "children" do
        ...
        context "with visible and invisible children" do
          let!(:child) do
            create(:work_package,
                   project:,
                   parent: work_package)
    ...
    context "when passing timestamps" do
      ...
      current_user do
        create(:user,
               firstname: "user",
               lastname: "1",
               member_with_permissions: { workspace => %i[view_work_packages view_file_links] })
      end
